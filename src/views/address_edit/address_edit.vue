  ...
  <div class="address_edit">
    ...
    <div class="fixBottom" >
      ...
      <div @click="deleteAddress">
        <span href="javescript:;" class="button">删除地址</span>
      </div>
    </div>
  ...
  export default{
    ...
    mounted:function(){
      ...
      axios.get(types.AREA).then(function (data) {
        ...
        if(this_.$route.query.addressId){//从地址编辑跳转过来
          let id=this_.$route.query.addressId;
          axios.get(types.ADDRESSLIST+'/'+id).then(function (data) {
            data=data.data;
            this_.address=data.address;
            this_.isDefault=data.isDefault;
            this_.phone=data.phone;
            this_.receiverName=data.receiverName;
            this_.provinceCode=data.provinceCode;
            this_.cityCode=data.cityCode;
            this_.areaCode=data.areaCode;
            this_.address=data.address;
            this_.id=data.id;
            let province=_.find(this_.data,{'code':this_.provinceCode});
            let city=_.find(province.children,{'code':this_.cityCode});
            if(city.children){
              var area=_.find(city.children,{'code':this_.areaCode});
            }
            if(area){
              this_.area=province.areaName+city.areaName+area.areaName
            }else{
              this_.area=province.areaName+city.areaName
            }
    ...
    methods: {
      ...
      add:function () {
        ...
        if (this_.receiverName==''){
          ...
          axios.post(types.ADDRESSLIST+'/'+this_.id,qs.stringify(data), {
            headers: {
            ...
            if(this_.$route.query){
              this_.$router.replace({path:'/address_list',query:this.$route.query})
            }else{
              this_.$router.replace({path:'/address_list'})
            }
      ...
      deleteAddress:function () {
          ...
          MessageBox.confirm('确定删除?').then(function (action) {
            axios.delete(types.ADDRESSLIST+'/'+this_.id).then(function(data){
              Toast('修改成功');
              if(this_.$route.query){
                this_.$router.push({path:'/address_list',query:this_.$route.query})
              }else{
                this_.$router.push({path:'/address_list'})
              }
            },function(data) {Toast(data.description)})
          });
        }else {
          Toast('默认地址不能删除');
        }
